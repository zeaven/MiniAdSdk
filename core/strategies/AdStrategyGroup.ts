...
export default class AdStrategyGroup implements IAdStrategyGroup{
    ...
    constructor(adConfig: RemoteAdConfigData, name?: string) {
        ...
        this._strategies = {}
        if (adConfig.nativeBannerStart || adConfig.nativeBannerGap) {
            const adStrategy: IAdStrategy = {adType: AdType.Banner}
        ...
        if (adConfig.interstitialStart || adConfig.interstitialGap) {
            const adStrategy: IAdStrategy = {adType: AdType.Interstitial}
        ...
        if (adConfig.nativeInterstitialStart || adConfig.nativeInterstitialGap) {
            const adStrategy: IAdStrategy = {adType: AdType.NativeInterstitial}
            ...
            adStrategy.nativeDownloadBtnOpacityRate = adConfig.willShowNativeDownloadTransparent * 100
            adStrategy.closeBtnScale = adConfig.nativeCloseButtonScale
        ...
        if (adConfig.nativeStart || adConfig.nativeGap) {
            const adStrategy: IAdStrategy = {adType: AdType.Native}
        ...
        if (adConfig.rewardStart || adConfig.rewardGap) {
            const adStrategy: IAdStrategy = {adType: AdType.Reward}
