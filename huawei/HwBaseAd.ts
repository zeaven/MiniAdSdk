...
export default abstract class HwBaseAd implements AdHandler {
  ...
  protected onError(err): void {
    HuaweiAd.log(this.name + '加载失败', JSON.stringify(err))
    this.reLoad(false)
  }

  protected onClose(res): void {
    HuaweiAd.log(this.name + '关闭')
    this.isShowed = false
    this.ready = false
